...
txt = txt
	...
	.replace(/Test: divisible by ([^\n]+)\n/g, 'num:$1, test: function(num){return (num%$1)===0 ?')
...
var modulus = monkeys.map(m=>m.num).reduce((a,b)=>a*b)
for(var r = 0; r<10000; r++){
	...
	monkeys.forEach(function(monkey, id){
		...
		for(var i = monkey.items.length - 1; i>=0; i--){
			var item = monkey.items[i];//.splice(i, 1)[0];
			var level = monkey.fn(item)%modulus;//3|0;
